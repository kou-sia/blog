...
積極的に採用したくなる返答をしてほしいというと大袈裟ですが、応募者に本来の力を発揮してもらえるよう会話を誘導したりはします。  
この記事に登場する会話は一例であり、実際の面接で直接話した内容ではありません。  
...
実際のチームではそういうことをする理由はないですが、面接だと選抜機能を持たせる必要があるので場合によっては行うことがあります。  
さらにこういう質問をする背景としては、入社された際には同じチームで働く可能性を想定しているからでもあり、あくまでも面接官と応募者は対等な存在であり面接官に誤りがあればそれは指摘をして欲しいという意図もあります。  
...
私が面接を行うときの判断基準は「チームで一緒に働けると感じるかどうか」です。  
「チームに居たら嬉しい方を採用すること」よりも「チームに居たら困る方を採用しないこと」を重要視しています。  
そのため「凄くできる人を採用する」ということは特に考えていないです。  

具体的な進め方ですが、以下の内容をどんどん掘り下げて聞いていきます。  
私の基準の場合、これだけでほぼ全ての応募者の採用判断が可能です。  
（もちろんカルチャーマッチなども重要ですが、それはあくまでオプションで、以下を満たした場合にのみ判断材料として追加して考えることにしています。）　 
...
これらの内容を適切に答えられる方であれば、カルチャー的に合わないと判断しない限りは採用基準を満たしたと判断します。  
これらを理解して実践できている方であれば、入社後に新しく使用するツールや技術なども問題なく習得できるだろうと判断しています。  

なお、中途採用の場合は過去の仕事を聞くことが多いです。  
過去の仕事内容が技術的に簡単な場合などで深掘りしようが無い場合は、[SystemDesign系面接](#systemdesign系面接)のような課題を出して技術力を判断します。  
(例えば多重下請け構造でエンジニアをされている方が応募されてくる場合などで、仕事内容が細分化された作業に終始しており説明を聞いても応募者本人の能力に対する判断材料として全く当てにならない場合があります。しかし、これは日本社会のせいであって応募者のせいではないので差し引いて判断して面接を行うようにしています。そもそもレジュメスクリーニングを突破しているということで、過去の仕事内容がどうなのかは面接での判断には直接関係がないと考えています。）


## 応募者の振る舞いで気付いた、できれば避けてほしいこと 

ここから、面接をしている中での応募者の振る舞いで気づいたこと（そしてできれば避けて欲しいこと）を羅列していきます。  
※ 私が「採用基準に満たない方を採用しないこと」を重要視している都合上、どうしてもネガティブな話ばかりになりますがご容赦ください。  
