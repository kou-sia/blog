...
※大前提  
わざわざ時間をかけて面接を受けていただいているのでできれば採用したいと思って面接しています。  
有効な返答というと大袈裟ですが、応募者が本来の力を発揮できるように会話を誘導したりはしています。  
...
面接方式はSystemDesignInterview系のやり方を採用しています。  
...
あくまでも面接官と応募者は対等な存在であり、ご縁があり入社された際には同じチームで働く可能性を想定していますので、面接官の誤りは指摘して欲しいという意図があります。  
...
- [面接官のコツというか感じたこととか](#面接官のコツというか感じたこととか)
  - [面接をしていてわかったこと](#面接をしていてわかったこと)
    ...
    - [応募者の一定数は、やりたいことができない存在しないロールに応募してくる](#応募者の一定数はやりたいことができない存在しないロールに応募してくる)
...
私が面接を行うときの判断基準は「チームメンバーとして一緒に働けるかどうか」です。  
基本的には以下をどんどん掘り下げていくだけで、ほとんどの応募者の採用判断が可能です。 
（もちろんカルチャーマッチなども重要ですが、それは以下を満たしてから判断します。）
...
(というのもこれは日本特有なのかは分かりませんが、多重下請け構造の元でエンジニアをされている方が応募されてくる場合など、過去の成果が細分化された作業に終始しており、説明を聞いても応募者本人の能力に対する判断材料として全く当てにならない場合が多々あります。しかし、これは日本社会のせいであって応募者のせいではないので、そこは差し引いて面接を行う必要があります。過去の仕事内容がどうなのかは採用可否判断には直接関係がないです。）

カルチャー的に合わないと判断しない限り、これらを正しく答えられれば面接で落とすことはないです。  
社内で使用しているツールなどはどうせ新しく学ぶことになるので、上記のようなことを理解して実践できていれば、おそらく新しい技術なども問題なく習得できるだろうと判断します。  
...
2. 応募者「xxが問題で処理が遅くなっていたのでDBにIndex張りました」、私「なぜそうしたのですか？」
   1. 「xxが問題で遅かったので対応するカラム△△にIndex張って速度改善しました」
      1. それは既に知ってる、なぜIndexを張るという解決方法を選んだのか聞いている
          1. 処理が遅いなら処理を分散したりパーティション分けたり、リアルタイム処理を諦めて夜間バッチ使ったりなど状況次第でやり方は色々ある
          2. そのやり方しか知らないからそのやり方を採用したと判断せざるを得ない回答を繰り返されるのは困る
      2. 理由を聞いているのに、機能や過程や結果を答える方は意外と多いです
...
1. 全体を見ていないので、どういう仕組みでシステム全体が動いているか全く知らない
   ...
   1. 自分が作った部分であっても単に作っただけで、なぜその機能が必要か、なぜそのアーキテクチャが採用されたのか理解していない
      1. 応募者「xx機能API作りました」、私「その機能はどういう用途ですか」、応募者「自分はバックエンドを作ってるだけでフロントのことは知らない」などと答える
      2. 自分の範囲以外は知らないとか言われると「あなた正社員採用受けにきたんですよね？単発作業依頼の応募じゃないですよ？」と思わざるを得ない

### 応募者の一定数は、自分がこれまで使ってきたプログラミング言語の特性の違いを理解していない

1. 応募者「CとPython使ってました」、私「CとPythonってそれぞれどういう特性がありますか？」と質問しても答えられない
   1. C言語を使っているのに、メモリ管理の話が一切でてこないと流石にヤバい
   2. Pythonを使っているのに「コンパイルできて早くていいですよね！」とかそれだけを言われると困る
      1. 今回の場合はC言語との差異も含めて聞いているので、c言語側の特性としてよく言われることを初手で言われると心配になる
      2. 別にPythonをコンパイルしようと思えばできますが、はっきり言って一般的ではないのでそこの補足はして欲しい

### 応募者の一定数は、分からないことを分からないと言えない

1. 私「○○機能を実装した時のアーキテクチャや実装方式を教えてください」、応募者「○○機能はxxデータを管理するもので、APIを使ってa,b,c項目をDBから取得します。DBは○○テーブルがあって、△△テーブルとJOINして...」と延々と機能フローを語る
   ...
   2. 「○○機能はAPIでDBに情報を読み書きします、またAPIの実装はNode.jsでトランザクションスクリプトで書いてます。またDBはリードレプリカを採用して負荷分散しています」くらいが返ってきてほしいです
      1. もちろん私からも「システムの構成全体を簡潔に表すためのアーキテクチャ図とかってありました？」などと聞きますが、分からない場合に同じ回答を連発される場合があります
   3. 分からないなら分からないで良いです。分からずに適当に設計実装される方が問題になるので、わからない場合はそれを正しく解答してもらえることも期待しています
      1. あまりにも基本的なことを知らない場合は困りますが、似たようなものを知ってたらそれを話してもらうでも良いです
      2. 「RDBはわかるけどKey Value Storeはあまり分からない」とかはありえます。
         1. 知っていてほしいものも多いですが、知らなくても少し説明して理解が出来るなら問題ないと判断できるので、関連知識があることはできれば見せてほしいです

### 新卒採用の応募者の一定数は、自身が行っている研究内容や専攻内容の詳細を答えることができない

1.  [「応募者の一定数は、自分が行ってきた作業が全体のどこに位置して何故その方式で作るのかを理解していない」](#応募者の一定数は自分が行ってきた作業が全体のどこに位置して何故その方式で作るのかを理解していない)と同様ではあるが、研究内容を質問された際の対策が不十分な応募者が多い
   1. 応募者「ロボット作ってました」
      1. 姿勢制御のことは聞くに決まってます
   2.  応募者「Deep Learning系の機械学習してました」
       1. ニューラルネットワークのことは聞くに決まってます
   4. こういった、一定確率で聞かれて、しかもその研究をしているのであれば当然答えられるべきことについては準備しておいてほしい
