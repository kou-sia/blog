...
（もちろんカルチャーマッチなども重要ですが、それは以下を満たした上で判断することにしています。）　 
...
これらを正しく答えられれば、カルチャー的に合わないと判断しない限りは面接で落とすことはないです。  
入社後に社内で使用するツールや技術などは新しく学ぶことになりますが、上記のようなことを理解して実践できていれば問題なく習得できるだろうと判断しています。  

中途採用の場合は過去の仕事を聞くことが多いです。  
過去の仕事内容が技術的に簡単な場合などで深掘りしようが無い場合は、[SystemDesign系面接](#systemdesign系面接)のような課題を出して技術力を判断します。  
(例えば多重下請け構造でエンジニアをされている方が応募されてくる場合などで、過去の成果が細分化された作業に終始しており説明を聞いても応募者本人の能力に対する判断材料として全く当てにならない場合が多々あります。しかしこれは日本社会のせいであって応募者のせいではないので、そこは差し引いて面接を行う必要があります。過去の仕事内容がどうなのかは採用可否判断には直接関係がないと考えているためです。）
...
2. 自分が作った部分であっても単に作っただけで、なぜその機能が必要か、なぜそのアーキテクチャが採用されたのか理解していない
   1. 応募者「xx機能API作りました」、私「その機能はどういう用途ですか」、応募者「自分はバックエンドを作ってるだけでフロントのことは知らない」などと答える
      1. 自分の範囲以外は知らないとか言われると「あなた正社員採用受けにきたんですよね？単発作業依頼の応募じゃないですよ？」と思わざるを得ない

### 応募者の一定数は、自分がこれまで使ってきたプログラミング言語の特性の違いを理解していない

1. 応募者「CとPython使ってました」、私「CとPythonnにはそれぞれどういう特性がありますか？」と質問しても答えられない
...
1. 私「○○機能を実装した時のアーキテクチャや実装方式を教えてください」、応募者「○○機能はxxデータを管理するもので、APIを使ってa,b,c項目をDBから取得します。DBは○○テーブルがあって、△△テーブルとJOINしてデータを取得します。それをSwaggerで決めた形式に整形して...」と延々と機能の処理フローを語る
   ...
   3. 分からないなら分からないで良い。分からずに適当に設計実装される方が問題になるので、わからない場合はわからないと解答してもらえることも期待している
      1. あまりにも基本的なことを知らない場合は困りますが、似たようなものを知ってたらそれを話すでも良い
      2. 「RDBはわかるけどKey Value Storeはあまり分からない」とかは経験上はありえる
         1. 知っていてほしいものも多いが、知らなくても少し説明して理解が出来るなら働く上では問題ないと判断できるので、関連知識があることはできれば見せてほしい
...
1.  [「応募者の一定数は、自分が行ってきた作業が全体のどこに位置して何故その方式で作るのかを理解していない」](#応募者の一定数は自分が行ってきた作業が全体のどこに位置して何故その方式で作るのかを理解していない)と同様ではあるが、研究内容を質問された際の対策が不十分な応募者が多い
   ...
   3. こういった、一定確率で聞かれてしかもその研究をしているのであれば当然答えられるべきことについては準備しておいてほしい
